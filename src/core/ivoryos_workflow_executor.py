...
class IvoryOSWorkflowExecutor:
    """Agent for executing Canvas workflows with SDL2."""

    def __init__(self, client: Optional[IvoryOSClient] = None):
        """
        ...
        """
        self.client = client or IvoryOSClient()
        self.workflow = None
        self.results = {}
    ...
    def execute_operation(self, operation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not len(operation_type.split(".")) == 2:
            raise ValueError(f"Operation type {operation_type} is not in device.method format")
        component, method = operation_type.split(".")

        logger.info(f"Executing operation {operation_id} of type {operation_type}")

        # Create and execute the appropriate unit operation
        result = self.client.execute_task(component, method, params)
...
def main():
    ...
    try:
        executor = IvoryOSWorkflowExecutor()
